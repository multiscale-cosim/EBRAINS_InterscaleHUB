...
import json
import time
import modify_tvb.noise as my_noise
import modify_tvb.Zerlaut as Zerlaut
from modify_tvb.Interface_co_simulation_parallel import Interface_co_simulation
from helper_function_zerlaut import findVec
...
def run_mpi(path):
    ...
    #init MPI :
    #WARNING here we only have one id_proxy (nest region) for the MVP
    ...
    while count*time_synch < end: # FAT END POINT
        ...
        # NOTE: this seems to be tvb side transformation and 'analysis' work
        # NOTE: this COULD be moved to the HUB as part of the outgoing transformer.
        # NOTE: receive 'data' via MPI and do the tvb simulation directly.
        #receive MPI data
        data_value = []
...
def init_mpi(path,logger):
    ...
    """
    '''
    # NOTE: commented out, not used (yet)
...
def send_mpi(comm,times,data):
    ...
    accept = False
    # NOTE: hardcoded MPI rank 0 as source (receive and send)
    # NOTE: needs work to be more flexible in the future.
    while not accept:
...
def receive_mpi(comm):
    ...
    """
    status_ = MPI.Status()
    #WARNING: dest and source changed to 1 (new HUB with hardcoded sending rank 1)
...
def end_mpi(comm,path,sending,logger):
    ...
    if sending :
        ...
        # WARNING: dest rankd changed to 1, new HUB (like above in sending function)
...
if __name__ == "__main__":
    ...
    #WARNING: hardcoded for one proxy/id/nest node
    import sys
    run_mpi(sys.argv[2]) # results_path
